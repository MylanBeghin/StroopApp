...
public class StroopViewModel : ViewModelBase
{
    ...
    private void GenerateTrials()
    {
        ...
        for (int i = 0 ; i < Settings.CurrentProfile.WordCount ; i++)
        {
            ...
            {
                var indices = Enumerable.Range(0, 4).OrderBy(_ => random.Next()).Take(2).ToArray();
    ...
    public async void StartTrials()
    {
        foreach (var trial in Settings.ExperimentContext.CurrentBlock.TrialRecords)
        {
            ...
            if (completed == _inputTcs.Task)
            {
                ...
                Settings.ExperimentContext.ReactionPoints.Add(new ReactionTimePoint(trial.TrialNumber, trial.ReactionTime, Settings.ExperimentContext.CurrentTrial.IsValidResponse));
                CurrentControl = new FixationCrossControl();
            }
            else
            {
